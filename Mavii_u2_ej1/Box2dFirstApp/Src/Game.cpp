...
void Game::DrawGame()
{
    ...
    sf::RectangleShape rectangle(sf::Vector2f(10.0f, 5.0f));
    rectangle.setFillColor(sf::Color::Cyan);
    rectangle.setPosition(rectangleBody->GetPosition().x, rectangleBody->GetPosition().y);
    rectangle.setOrigin(Vector2f(5.00, 2.5));
    wnd->draw(rectangle);
...
void Game::InitPhysics()
{
    ...
    //fisicas del rectangulo

    rectangleBody = Box2DHelper::CreateRectangularDynamicBody(phyWorld, 10.0f, 5.0f, 1.0f, 0.5f, 0.1f);
    rectangleBody->SetTransform(b2Vec2(50.0f, 50.0f), 0.0f);
}
